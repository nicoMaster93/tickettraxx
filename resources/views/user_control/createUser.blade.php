        ...
        <form method="POST" action="{{ route('users.create') }}" enctype="multipart/form-data">
                        ...
                        @foreach ($arrMenu as $menu)
                            <li>
                                <div class="form-check">
                                    <input class="form-check-input" checked type="checkbox" name="permiso[]" value="{{$menu["menu"]->id}}" id="permiso_{{$menu["menu"]->id}}" /> 
                                ...
                                @if (sizeof($menu["subItems"]) > 0)
                                    ...
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input" checked type="checkbox"  name="permiso[]" value="{{$menulv2["menu"]->id}}" id="permiso_{{$menulv2["menu"]->id}}"  /> 
                                        ...
                                        @if (sizeof($menulv2["subItems"]) > 0)
                                            ...
                                            @foreach ($menulv2["subItems"] as $menulv3)
                                                <li>
                                                    <div class="form-check">
                                                        <input class="form-check-input" checked type="checkbox" name="permiso[]" value="{{$menulv3["menu"]->id}}" id="permiso_{{$menulv3["menu"]->id}}" />
            ...
            <footer class="form-group">
                <div class="text-center">
                    <button type="submit" class="btn btn-verde2">Create User</button>
                </div>
            </footer>
