...
<div class="container-fluid min-h-100-vh">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{route('drivers_contractor.index')}}">Drivers</a></li>
            <li class="breadcrumb-item active" aria-current="page">Edit</li>
        </ol>
    </nav>
    <div class="form-content">
        <form method="POST" action="{{ route('drivers_contractor.edit', ['id' => $driver->id]) }}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-3">
                    <div class="form-floating">
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="Name" value="{{ old('name', $driver->name) }}" required>
                        ...
                        <select class="form-control form-select select-with-other find-cities @error('state') is-invalid @enderror" 
                         ...
                         id="state" name="state" required>
                            ...
                            @foreach ($location_states as $location_state)
                                <option value="{{$location_state->id}}" @if (old('state', $driver->location_city->fk_location) == $location_state->id) selected @endif>{{$location_state->location_name}}</option>
                            @endforeach                            
                        ...
                        <select class="form-control form-select select-with-other @error('city') is-invalid @enderror" 
                         ...
                         id="city" name="city" required>
                            ...
                            @foreach ($location_cities as $location_city)
                                <option value="{{$location_city->id}}" @if (old('city',$driver->location_city->id) == $location_city->id) selected @endif>{{$location_city->location_name}}</option>
                            @endforeach
                ...
                <div class="col-md-3">
                    <div class="form-floating">
                        <input type="text" class="form-control @error('address') is-invalid @enderror" id="address" name="address" value="{{ old('address', $driver->address) }}" placeholder="Address" required>
                ...
                <div class="col-md-3">
                    <div class="form-floating">
                        <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $driver->email) }}" placeholder="Email" required>
                ...
                <div class="col-md-3">
                    <div class="form-floating">
                        <input type="text" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', $driver->phone) }}" placeholder="Phone" required>
            ...
            <div class="row">               
                <div class="col-12 preview">
                    ...
                    <img @if(Storage::disk('local')->exists($driver->photo_cdl))
                            src="{{Storage::url($driver->photo_cdl)}}"
                         @else
            ...
            <div class="row">               
                <div class="col-12 preview">
                    ...
                    <img @if(Storage::disk('local')->exists($driver->photo_medical_card))
                            src="{{Storage::url($driver->photo_medical_card)}}"
                         @else
                            src="/imgs/theme/no-image.png"
                         @endif   
                    id="preview_medical_card" />
                </div>
                <span id="preview_medical_card_text" class="col-3" style="display: none;"></span>
            </div>
            <footer class="form-group">
                <div class="text-center">
                    <button type="submit" class="btn btn-verde2">Edit Driver</button>
                </div>
            </footer>
